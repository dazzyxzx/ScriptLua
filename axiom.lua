...
-- ===== lógica real do Steal an Egg (do DiceHub) =====
...
local function eggInfo(egg)
    ...
    return {rar=rar, mut=egg.Mutations, val=val}
end

local function eggIcon(egg)
    local m = egg.PhysicalModel
    if not m then return "" end
    for _, d in ipairs(m:GetDescendants()) do
        if d:IsA("Decal") and d.Texture ~= "" then return d.Texture end
        if d:IsA("Texture") and d.Texture ~= "" then return d.Texture end
    end
    for _, d in ipairs(m:GetDescendants()) do
        if d:IsA("BasePart") and d:FindFirstChildOfClass("Decal") then
            return d:FindFirstChildOfClass("Decal").Texture
        end
    end
    return ""
end

local function steal(egg)
    local m = egg.PhysicalModel
    if not m then return end
    for _, d in ipairs(m:GetDescendants()) do
...
local nameLbl = n("TextLabel",{
    BackgroundTransparency = 1,
    Position = UDim2.fromOffset(64,22),
...
local function addRow(idx, egg)
    ...
    n("TextLabel",{
        ...
        Font = Ft.b,
        Text = "#"..idx,
        TextColor3 = idx == 1 and Color3.fromRGB(230,190,90) or C.dim,
        TextSize = 10,
        TextXAlignment = Enum.TextXAlignment.Left,
        ZIndex = 4,
    }, row)

    local ico = n("ImageLabel",{
        BackgroundColor3 = C.lift,
    ...
    n("TextLabel",{
        ...
        Size = UDim2.new(1,-110,0,12),
        Font = Ft.b,
        Text = egg.AssetCategory or egg.Uid or "Egg",
        TextColor3 = C.txt,
        TextSize = 10,
        TextTruncate = Enum.TextTruncate.AtEnd,
        TextXAlignment = Enum.TextXAlignment.Left,
    ...
    local sub = i.rar .. (i.mut and (" - "..tostring(i.mut)) or "")
    n("TextLabel",{
        BackgroundTransparency = 1,
        ...
        Font = Ft.m,
        Text = sub,
        TextColor3 = RAR_COLOR[i.rar] or C.mute,
        TextSize = 8,
        TextXAlignment = Enum.TextXAlignment.Left,
        ZIndex = 4,
    }, row)
...
task.spawn(function()
    while task.wait(1) do
        if not char or not char.Parent then
            repeat task.wait(0.2) until LP.Character
        ...
        if #eggs > 0 then
            local top = eggs[1]
            local i = eggInfo(top)
            nameLbl.Text = top.AssetCategory or top.Uid or "Egg"
            mutLbl.Text  = i.rar .. (i.mut and (" - "..tostring(i.mut)) or "")
            mutLbl.TextColor3 = RAR_COLOR[i.rar] or C.purple
            valLbl.Text  = fmt(i.val)
            local icon = eggIcon(top)
            if icon ~= "" then mi.Image = icon end
        else
        ...
        -- lista
        clearRows()
        for idx = 1, math.min(5, #eggs) do
            addRow(idx, eggs[idx])
        end

        -- auto steal
        if Auto and #eggs > 0 then
            local target = eggs[1]
            ...
            elseif target.PhysicalModel then
                local ok, pos = pcall(function()
                    return target.PhysicalModel:GetPivot().Position
                end)
                if ok and pos then
                    pcall(function()
